...
const products = [
  ...
  { id: 9, name: "Purocis RO+UV 8L", description: "Efficient RO+UV system with advanced filtration and reliable daily performance.", price: 11999, originalPrice: 13999, image: "/images/purosisro.jpg", category: "purifier", efficiency: "8 L | 18 LPH", warranty: "1 Year", features: ["RO+UV combo","Sediment + carbon stages","Low TDS compatibility","Serviceable filters","Compact body"] },
  ...
  return (
    <section id="products" className="py-14 sm:py-16 bg-gradient-to-br from-blue-50 via-white to-green-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            return (
              ...
              >
                <div className="relative overflow-hidden h-44 sm:h-48 md:h-52 bg-white">
                  <img src={product.image} alt={product.name} className="w-full h-full object-contain object-center transition-transform duration-300" loading="lazy" />
                  ...
                  <div className={`pointer-events-none absolute inset-0 bg-gradient-to-t ${getCategoryColor(product.category)} opacity-0 group-hover:opacity-10 transition-opacity duration-300`} />
                </div>
                ...
                <div className="p-5 sm:p-6">
                  ...
                  <button className={`w-full bg-gradient-to-r ${getCategoryColor(product.category)} text-white font-semibold py-2.5 sm:py-3 px-4 rounded-lg hover:shadow-lg transition-all duration-300 text-sm`}>
                    View Details
                  </button>
                </div>
              </div>
          ...
          <section className="mt-14 sm:mt-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {commercialOnly.map((product) => {
                ...
                return (
                  <div key={product.id} className="bg-white rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden group hover:-translate-y-1.5">
                    <div className="relative overflow-hidden h-44 sm:h-48 md:h-52 bg-white">
                      <img src={product.image} alt={product.name} className="w-full h-full object-contain object-center" loading="lazy" />
                      <div className={`absolute top-3 left-3 px-3 py-1 rounded-full text-xs sm:text-sm font-semibold ${badge.bg}`}>
                        {badge.icon} {badge.label}
                      </div>
                      <div className={`pointer-events-none absolute inset-0 bg-gradient-to-t ${getCategoryColor(product.category)} opacity-0 group-hover:opacity-10 transition-opacity duration-300`} />
                    ...
                    <div className="p-5 sm:p-6">
                      ...
                      {/* Price only (no MRP/savings in commercial) */}
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-xl sm:text-2xl font-bold text-gray-900">₹{product.price.toLocaleString()}</span>
                      </div>
                      ...
                      <div className="flex gap-3">
                        <button
                          onClick={() => setSelectedProduct(product)}
                          className={`flex-1 bg-gradient-to-r ${getCategoryColor(product.category)} text-white font-semibold py-2.5 sm:py-3 px-4 rounded-lg hover:shadow-lg transition-all duration-300 text-sm`}
                        >
                          View Details
                        </button>
                        <button
          ...
          <div className="rounded-3xl overflow-hidden bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 p-[1px]">
            <div className="rounded-3xl bg-white">
              <div className="px-5 sm:px-8 py-8 sm:py-10">
                <h3 className="text-2xl sm:text-3xl font-extrabold text-gray-900 text-center mb-6">Why Choose Bhairava</h3>
                ...
                <div className="mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                  <button onClick={() => setContactModalOpen(true)} className="px-5 sm:px-7 py-3 rounded-xl bg-white text-indigo-700 font-semibold shadow hover:shadow-md border border-indigo-200">
                    Get a Free Quote
                  </button>
                  <a href="tel:+918310280310" className="px-5 sm:px-7 py-3 rounded-xl bg-indigo-700 text-white font-semibold shadow hover:shadow-md text-center">
                    📞 Call Now
                  </a>
                </div>
              </div>
        ...
        {/* Modal (no variant options displayed) */}
        {selectedProduct && (
          <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4 backdrop-blur-sm">
            <div className="relative bg-white rounded-t-2xl sm:rounded-2xl overflow-hidden w-full sm:max-w-2xl max-h-[92vh] sm:max-h-[90vh] overflow-y-auto">
              ...
              <div className="relative h-52 sm:h-64 bg-white">
                <img src={selectedProduct.image} alt={selectedProduct.name} className="w-full h-full object-contain object-center" />
              ...
              <div className="p-6 sm:p-8">
                <h3 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3 sm:mb-4">{selectedProduct.name}</h3>
                <p className="text-gray-700 text-base sm:text-lg mb-5 sm:mb-6 leading-relaxed">{selectedProduct.description}</p>
                ...
                <div className="border-t pt-5 sm:pt-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                    <button
                      onClick={() => setContactModalOpen(true)}
                      className={`flex-1 bg-gradient-to-r ${getCategoryColor(selectedProduct.category)} text-white font-bold py-3 sm:py-4 px-6 sm:px-8 rounded-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 text-center`}
                    >
                      Inquire Us
                    </button>
                    <a
